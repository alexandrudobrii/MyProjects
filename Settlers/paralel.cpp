...
void calculate() {
	int pret_maxA, pret_maxB, cost, cost_res, i, j;
	int i1, j1;

	for (int an = 0; an < iterations; an++) {
		
		/* se updateaza matricile in paralel cu valorile din noul an */
		#pragma omp paralell private(i1, j1)
		for (i1 = 0; i1 < n; i1++) {
			for (j1 = 0; j1 < n; j1++) {
		...
		/* se paralelizeaza parcurgerea pe linii a matricilor */
		#pragma omp parallel for private(i, j, cost, cost_res) 
		for (i = 0; i < n; i++) {
			for (j = 0; j < n; j++) {
